        ...
        if ((newYmax==-Inf)) {
          ...
          if (bySampled & markSampled) {
            ...
            if(nSampled>0){
              ...
              if(plotType!="DiscProvided"){
                text(b[length(b)]+(b[2]-b[1])/4,newYmax*104/106,"sampled",cex=0.8)
              }else{
                text(b[length(b)]+(b[2]-b[1])/4,newYmax*104/106,"Landings with Discards",cex=0.8)
              }
            }
          }
...
################################## survey consistency function ###############################
cor.tun <- function(stk.tun){ for(i in names(stk.tun)) 
  if(dim(stk.tun[[i]]@index)[1]>1) plot(stk.tun[[i]],type="internal",main=name(stk.tun[[i]]))}
